...
namespace Calculator.Logic.CommandLineParser
{
    public static class CommandLineParserCreator
    {
        public static FluentCommandLineParser ArgumentsSetup(ApplicationArguments args)
        {
            ...
            parser.Setup<bool>(ParserShortAndLongNames.ToDegreeShort, ParserShortAndLongNames.ToDegreeLong).Callback(arg => args.ToDegree = arg);
            parser.Setup<string>(ParserShortAndLongNames.UnitConversionShort, ParserShortAndLongNames.UnitConversionLong).Callback(arg => args.UnitForConversion = arg);
