...
namespace Calculator.Logic
{
    ...
    /// </summary>
    public class ModelBuilder : ITokenVisitor
    {
        ...
        public void Visit(VariableToken variableToken)
            => Add(new Variable {Variables = variableToken.Variable});
        ...
        static int FindExpressionIndexForParenthesesIndex(int parenthesisIndex, ParenthesesNode child)
        {
            ...
            for (var expressionIndex = 0; expressionIndex < child.Parent.Expressions.Count; expressionIndex++)
            {
                if (child.Parent.Expressions[expressionIndex] is ParenthesedExpression)
                    parenthesisCount++;
                if (parenthesisCount == parenthesisIndex)
            ...
            where V : IArithmeticOperation
        {
            ...
            {
                if (expressions[i] is U)
                    HandleOperation<U>(expressions, i);
                else if (expressions[i] is V)
                    HandleOperation<V>(expressions, i);
                else
                    ++i;
            }
        }
        ...
        {
            if (IsWrapped)
                mNode.Expressions.Add(expression);
            else
